...
SESSION_FIELDS = [
    ...
    "Duration_Seconds","Duration","Logon_ID","Parent_Process","Exit_Status"
]
SUMMARY_FIELDS = [
    "User","Computer","Application","Sessions_Last_30_Days",
    "Total_Usage_Last_30_Days",
    "First_Usage_IST","Last_Usage_IST"
]
...
def summary_file(app):
    p=OUT/safe(app); p.mkdir(parents=True,exist_ok=True)
    return p/f"{safe(app)}_30Day_Summary.csv"
...
def rebuild_summary(app):
    src=session_file(app); dst=summary_file(app)
    if not src.exists(): return
    cutoff=datetime.now(timezone.utc)-timedelta(days=DAYS)
    groups=defaultdict(lambda:{"n":0,"sec":0.0,"first":None,"last":None})
    with src.open("r",newline="",encoding="utf-8-sig") as f:
        for r in csv.DictReader(f):
            try:
                st=datetime.strptime(r["Start_Time_IST"],"%Y-%m-%d %H:%M:%S.%f").replace(
                    tzinfo=timezone(timedelta(hours=5,minutes=30)))
                en=datetime.strptime(r["End_Time_IST"],"%Y-%m-%d %H:%M:%S.%f").replace(
                    tzinfo=timezone(timedelta(hours=5,minutes=30)))
                if st.astimezone(timezone.utc)<cutoff: continue
                key=(r["User"],r["Computer"],r["Application"]); g=groups[key]
                g["n"]+=1; g["sec"]+=float(r["Duration_Seconds"])
                g["first"]=st if g["first"] is None or st<g["first"] else g["first"]
                g["last"]=en if g["last"] is None or en>g["last"] else g["last"]
            except (KeyError,ValueError): pass
    with dst.open("w",newline="",encoding="utf-8-sig") as f:
        w=csv.DictWriter(f,fieldnames=SUMMARY_FIELDS); w.writeheader()
        for (u,c,a),g in sorted(groups.items()):
            w.writerow({"User":u,"Computer":c,"Application":a,
                "Sessions_Last_30_Days":g["n"],
                "Total_Usage_Last_30_Days":dur(g["sec"]),
                "First_Usage_IST":g["first"].strftime("%Y-%m-%d %H:%M:%S.%f")[:-3],
                "Last_Usage_IST":g["last"].strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]})
...
def handle(e,st,apps):
    ...
    if e["id"]==4688:
        ...
        row={"Start_EventRecordID":start["rid"],"End_EventRecordID":e["rid"],
            ...
            "Parent_Process":start["parent"],"Exit_Status":d.get("Status","")}
...
def main():
    ...
    print("Output:",OUT); print("Rolling summary:",DAYS,"days"); print("Ctrl+C to stop.")
    while True:
        try:
